...
- ✅ Offline detection + UI gating — `useOnlineStatus` backed by `src/lib/connectivity.ts`: effective online = `navigator.onLine` **AND** server reachable (API transport errors flip offline instantly via `baseApi`; `GET /health` heartbeat every 30 s catches silent loss, 5 s retries while offline detect recovery — handles the router-up/internet-down case `navigator.onLine` misses); plan-aware `OfflineBanner`; `ProductSearch`/`BatchPicker` read the **cached** catalog (`db/catalog.ts` via `dexie-react-hooks` `useLiveQuery`) when offline; server-only actions disabled offline (Recent sales, customer attach, finalize) with inline hints
